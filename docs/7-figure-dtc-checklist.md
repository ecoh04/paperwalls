...
>
> **✅ Test order PW-1023 (2026-06-18) — CAPI VERIFIED WORKING:** first-party funnel captured the **full 8-stage journey** end to end; PayFast ITN paid correctly; and Events Manager (dataset 1555913832788701) shows **Purchase + InitiateCheckout via Conversions API, Active, EMQ 8.0/10** + 6 browser PageViews. The earlier "blocked on an env var" read was WRONG (a tripped guard would have sent Meta nothing). `capi_events` only *looked* empty because the audit insert was a fire-and-forget `void` write killed by the serverless freeze (same bug as commit `1fe6880`, never applied to `capi.ts`) — **FIXED**: the insert is now awaited + logs on error. Remaining match-quality items (non-blocking, EMQ already 8.0 without them): fbp/fbc absent (mobile blocked the pixel; desktop re-test), dataset shows "No websites found" (verify + connect domain), Purchase UA via a `sessions` join that may be null.
...
1. **Meta CAPI is VERIFIED WORKING** (Events Manager: Purchase + InitiateCheckout via CAPI, EMQ 8.0/10). The empty `capi_events` was a fire-and-forget audit-insert killed by serverless, now **fixed** (awaited + error-logged). Remaining hardening, all non-blocking, do before scaling ad spend: connect + verify the production domain to the dataset ("No websites found"); confirm `META_CAPI_TEST_EVENT_CODE` is NOT set in Production; desktop re-test to confirm `_fbp/_fbc` capture (lifts EMQ); store buyer UA on the order so Purchase CAPI doesn't depend on a nullable `sessions` join; gate `ok` on `events_received>=1` (not just HTTP 200). · S
...
**Acquisition** — Meta Pixel + CAPI VERIFIED LIVE (Events Manager: Purchase + InitiateCheckout via CAPI, EMQ 8.0/10) `[DONE] P0`; `_fbp/_fbc` + buyer-IP capture into CAPI — IP confirmed; fbp/fbc absent on PW-1023 (mobile likely blocked the pixel), desktop re-test pending to lift EMQ `[PARTIAL] P1`; `CustomizeProduct` configurator event `[DONE]`; Graph API pinned to v22 (v19 expired) `[DONE]`; Google Ads via stored `gclid` `[PARTIAL] P1`; monthly UGC/creative engine (3–5 SA micro-creators @ R1.5k–R5k) `[TODO] P1`; value-based lookalikes from `customers` `[PARTIAL] P1`; Product/Org JSON-LD shipped, Search Console verify pending `[PARTIAL] P1`.
...
**Data & Analytics** — full-journey first-party funnel + **8-stage sequential** admin funnel `[DONE] P0` (PW-1023 confirmed the whole journey captures live); analytics **command center** (MTD goal/run-rate, auto "what to do" insights, by-source funnel, channel table, sample-pack panel) `[DONE] P0`; Meta server CAPI **verified live** (EMQ 8.0); the `capi_events` audit insert was fire-and-forget and killed by serverless → now awaited + error-logged so the table reflects reality `[DONE] P0`; contribution-margin engine **scaffolded** (set unit costs in `analytics-config.ts` to light it up) `[PARTIAL] P0`; CAC/ROAS/LTV:CAC layer + manual `ad_spend` table `[TODO] P0`; server-side ViewContent/AddToCart CAPI mirror `[TODO] P1` (after rate-limiting); join events→customers + cohort/repeat view `[PARTIAL] P1`; CAPI audit trail in `capi_events` `[DONE]`; POPIA-safe analytics retention `[PARTIAL] P2`.
