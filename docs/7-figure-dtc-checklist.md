...
>
> **⚠️ Test order PW-1023 (2026-06-18 07:30) — results:** first-party funnel captured the **full 8-stage journey** end to end (sample-add + checkout.started fixes both confirmed live); PayFast ITN paid correctly; client Pixel confirmed live in prod HTML; buyer IP captured. **BUT server CAPI sent nothing — `capi_events` = 0 rows.** Root cause: `sendMetaConversion()` returns at its env guard (`capi.ts:104`) before the audit insert, and an empty table (vs a `failed` row) proves the guard tripped → the SERVER vars `META_PIXEL_ID` (the plain, non-`NEXT_PUBLIC_` twin) and/or `META_CAPI_ACCESS_TOKEN` are missing / not scoped to Production in Vercel. Code is correct; this is an env-var + redeploy fix. fbp/fbc also absent on the order (mobile likely blocked `connect.facebook.net`; needs a desktop re-test).
...
1. **FIX Meta server CAPI (BLOCKED — test order PW-1023 proved it sends nothing).** `capi_events` = 0 rows ⇒ the env guard tripped. In Vercel confirm BOTH `META_PIXEL_ID` = `1555913832788701` (Sensitive OFF) AND `META_CAPI_ACCESS_TOKEN` (Sensitive ON) exist for the **Production** environment, then **redeploy** (runtime env snapshots at build). Re-test → confirm `Purchase` + `InitiateCheckout` rows `status='sent'` in `capi_events` + Events Manager "deduplicated" + EMQ 7+. Quick win first: log `sendMetaConversion()`'s `{ok, reason}` at both call sites so the next test names the failure. · S
...
**Acquisition** — Meta Pixel+CAPI wired+deployed; client Pixel live, but server CAPI BLOCKED on a Production env var (PW-1023 sent 0 CAPI events) `[BLOCKED] P0`; `_fbp/_fbc` + buyer-IP capture into CAPI — IP confirmed; fbp/fbc absent on PW-1023 (mobile likely blocked the pixel), desktop re-test pending `[PARTIAL] P0`; `CustomizeProduct` configurator event `[DONE]`; Graph API pinned to v22 (v19 expired) `[DONE]`; Google Ads via stored `gclid` `[PARTIAL] P1`; monthly UGC/creative engine (3–5 SA micro-creators @ R1.5k–R5k) `[TODO] P1`; value-based lookalikes from `customers` `[PARTIAL] P1`; Product/Org JSON-LD shipped, Search Console verify pending `[PARTIAL] P1`.
...
**Data & Analytics** — full-journey first-party funnel + **8-stage sequential** admin funnel `[DONE] P0` (PW-1023 confirmed the whole journey captures live); analytics **command center** (MTD goal/run-rate, auto "what to do" insights, by-source funnel, channel table, sample-pack panel) `[DONE] P0`; Meta server CAPI **NOT sending** — `capi_events` empty, env guard tripped, fix `META_PIXEL_ID`/`META_CAPI_ACCESS_TOKEN` in Vercel + redeploy `[BLOCKED] P0`; contribution-margin engine **scaffolded** (set unit costs in `analytics-config.ts` to light it up) `[PARTIAL] P0`; CAC/ROAS/LTV:CAC layer + manual `ad_spend` table `[TODO] P0`; server-side ViewContent/AddToCart CAPI mirror `[TODO] P1` (after rate-limiting); join events→customers + cohort/repeat view `[PARTIAL] P1`; CAPI audit trail in `capi_events` `[DONE]`; POPIA-safe analytics retention `[PARTIAL] P2`.
